...
class IDS_District_List_Table extends WP_List_Table {
  ...
  // This method is used to search of the value form database   
    if(!empty($_REQUEST['s'])){
      $queryPart = "
        WHERE
          `".$district."`.`title` LIKE '%".$_REQUEST['s']."%'
        OR
          `".$country."`.title    LIKE '%".$_REQUEST['s']."%' 
        OR
          `".$state."`.title      LIKE '%".$_REQUEST['s']."%'   
      ";
    }  
    if(!empty($orderby)){
    ...
    $fatchQuery = $wpdb->get_results($query);  
     foreach ($fatchQuery as $value) {
      $response[] = array(
        'id'          => $value->id, 
        'title'       => '<a class="row-title" href="'.admin_url('admin.php?page=add-new-district','admin').'&post='. $value->id .'&amp;action=edit">'.$value->title.'</a>',
