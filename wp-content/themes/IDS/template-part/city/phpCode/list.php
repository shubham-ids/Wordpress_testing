...
class IDS_District_List_Table extends WP_List_Table {
  ...
  function custom_record(){
    ...
    $searchBar    = (isset($_REQUEST['s']) ) ? $_REQUEST['s']   : '';   
  ...
  // This method is used to search of the value form database   
    ...
    $fatchQuery = $wpdb->get_results($query);  
     foreach ($fatchQuery as $value) {
      $response[] = array(
        'id'          => $value->id, 
        'title'       => '<a class="row-title" href="'.admin_url('admin.php?page=add-new-city','admin').'&post='. $value->id .'&amp;action=edit">'.$value->title.'</a>',
        'description' => mb_strimwidth($value->description, 0, 30, "....."),
        'country_id'  => $value->cTitle,
        'state_id'    => $value->sTitle,
        'district_id' => $value->dTitle,
    ...
    function get_sortable_columns() {
        $sortable_columns = array(
            ...
            'country_id'   => array('country_id',false),
            'state_id'     => array('state_id',false),
            'district_id'  => array('district_id',false),
      ...
      //Detect when a bulk action is being triggered...
        if( 'deleted' === $this->current_action() ) {
          foreach ($_REQUEST['user'] as $id) {
            $deleteQuery = DeleteAction( STATE , $id);
            if($deleteQuery === 0){
