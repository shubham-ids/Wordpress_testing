...
class IDS_state_List_Table extends WP_List_Table {
  ...
  // This method is used to search of the value form database   
    if(!empty($_REQUEST['s'])){
      $queryPart = "
      WHERE
        `".$T1."`.`title` LIKE '%".$_REQUEST['s']."%'
      OR 
        `".$T2."`.`title` LIKE '%".$_REQUEST['s']."%' 
      ";
      echo $queryPart;
    }  
    if(!empty($orderby)){
  ...
  $query = "
   ...
   $fatchQuery = $wpdb->get_results($query);  
     foreach ($fatchQuery as $value) {
      $response[] = array(
          'id'          => $value->id, 
          'title'       => '<a class="row-title" href="'.admin_url('admin.php?page=add-new-state','admin').'&post='. $value->id .'&amp;action=edit">'.$value->title.'</a>',
