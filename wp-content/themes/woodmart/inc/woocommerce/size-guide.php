        ...
        ?>
            <select name="woodmart_sguide_select">
                ...
                <?php foreach ( $sguide_list as $sguide_post ): ?>
                    <option value="<?php echo $sguide_post->ID; ?>" <?php selected( $sguide_post_id, $sguide_post->ID ); ?>><?php echo $sguide_post->post_title; ?></option>
                <?php endforeach; ?>
...
if( ! function_exists( 'woodmart_sguide_dropdown_save' ) ) {
    function woodmart_sguide_dropdown_save( $post_id ){
        if ( isset( $_POST['woodmart_sguide_select'] ) ) {
            ...
            if ( isset( $_POST['woodmart_disable_sguide'] ) && $_POST['woodmart_disable_sguide'] == 'on' ) {
                ...
                update_post_meta( $post_id, 'woodmart_sguide_select', $_POST['woodmart_sguide_select'] );
            }
        ...
        if ( isset( $sguide_post_id[0] ) && !empty( $sguide_post_id[0] ) ){
            ...
            if ( $terms ) {
                foreach( $terms as $term ){
                    if ( get_woocommerce_term_meta( $term->term_id, 'woodmart_chosen_sguide' ) ) {
                        $sguide_post_id = get_woocommerce_term_meta( $term->term_id, 'woodmart_chosen_sguide' );
                    }else{
    ...
    function woodmart_sguide_display_table_template( $sguide_post, $size_tables ){
        ...
        ?>
            ...
            <div id="woodmart_sizeguide" class="mfp-with-anim woodmart-content-popup mfp-hide woodmart-sizeguide">
                <h4 class="woodmart-sizeguide-title"><?php echo esc_html( $sguide_post->post_title ); ?></h4>
                <div class="woodmart-sizeguide-content"><?php echo do_shortcode( $sguide_post->post_content ); ?></div>
...
if( ! function_exists( 'woodmart_sguide_save_category' ) ) {
    function woodmart_sguide_save_category( $post_id ) {
        if ( isset( $_POST['woodmart_sguide_category'] ) ) {
            $selected_sguide_category = $_POST['woodmart_sguide_category'];
            ...
            foreach ( $selected_sguide_category as $selected_sguide_cat ) {
                update_woocommerce_term_meta( $selected_sguide_cat, 'woodmart_chosen_sguide', $post_id );
            }   
            foreach( $terms as $term ){
                if ( !in_array( $term->term_id, $selected_sguide_category ) ) {
                    if ( $post_id == get_woocommerce_term_meta( $term->term_id, 'woodmart_chosen_sguide' ) ) {
                        update_woocommerce_term_meta( $term->term_id, 'woodmart_chosen_sguide', '' );
                    }
                }
            }
        }
    ...
    function woodmart_sguide_category_template( $post ) {
        ...
        <ul>
            <?php foreach ( $sguide_cat_list as $sguide_cat ): ?>
                ...
                <li>
                    <input type="checkbox" name="woodmart_sguide_category[]" value="<?php echo $sguide_cat->term_id; ?>" <?php echo $checked; ?>>
                    <?php echo $sguide_cat->name; ?>
                </li>
            <?php endforeach; ?>
        </ul>
        <?php
