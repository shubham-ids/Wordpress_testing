			...
			<?php
				...
				foreach ( $grouped_products as $grouped_product ) {
					...
					foreach ( $grouped_product_columns as $column_id ) {
						...
						switch ( $column_id ) {
							case 'thumbnail' :
								$attachment_id = get_post_meta( $grouped_product->get_id(), '_thumbnail_id', true );
								$value = '<td class="product-thumbnail grouped-thumb">';
									$value .= wp_get_attachment_image( $attachment_id, 'shop_thumbnail' );	
								$value .= '</td>';
								break;
							case 'label' :
							...
							case 'quantity' :
                                ...
                                echo '<td class="product-quantity" data-title="' . esc_attr( 'Quantity', 'woocommerce' ) . '">';
								...
								if ( ! $grouped_product->is_purchasable() || $grouped_product->has_options() || ! $grouped_product->is_in_stock() ) {
									...
									woocommerce_quantity_input( array(
										...
										'input_value' => isset( $_POST['quantity'][ $grouped_product->get_id() ] ) ? wc_stock_amount( wc_clean( wp_unslash( $_POST['quantity'][ $grouped_product->get_id() ] ) ) ) : 0, 
								...
								break;
							case 'price' :
                                $value = '<td class="price" data-title="' . esc_attr( 'Price', 'woocommerce' ) . '">';
    							$value .= '<div>';
